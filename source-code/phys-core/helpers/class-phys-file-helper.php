...
/**
 * Class Phys_File_Helper.
 *
 * @since 1.0.1
 */
if ( ! class_exists( 'Phys_File_Helper' ) ) {

	class Phys_File_Helper {
		public static $tag = 'Phys_File_Helper';
		...
		private static function call_wp_file_system() {
			...
			if ( empty( $wp_filesystem ) ) {
				...
				if ( ! $connect ) {
					_doing_it_wrong( __FUNCTION__, __( 'Connect to WP Filesystem failed!', 'phys-core' ), PHYS_CORE_VERSION );
				}
			}
		}
		...
		public static function rmdir( $dir ) {
			...
			foreach ( scandir( $dir ) as $item ) {
				...
				if ( ! Phys_File_Helper::rmdir( $dir . DIRECTORY_SEPARATOR . $item ) ) {
					return false;
				}
			}
		...
		public static function write( $path, $content ) {
			$out_fp = fopen( $path, 'w' );
			if ( ! $out_fp ) {
				return new WP_Error( self::$tag, __( 'Can not save file', 'phys-core' ) );
			}
		...
		public static function download_file( $file ) {
			...
			if ( empty( $file ) ) {
				return new WP_Error( 'empty_file', __( 'Empty file.', 'phys-core' ) );
			}
