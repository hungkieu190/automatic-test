...
/**
 * Class Phys_Template_Helper.
 *
 * @since 0.9.0
 *
 */
if ( ! class_exists( 'Phys_Template_Helper' ) ) {
	class Phys_Template_Helper {
		/**
		 * Get template path.
		 *
		...
		private static function get_template_path() {
			return apply_filters( 'phys_core_template_path', 'phys-core/' );
		}
		...
		public static function template( $path, $args = array(), $render = false ) {
			...
			if ( ! $template ) {
				$template = PHYS_CORE_PATH . '/templates/' . $path;
			}
