...
//$url_auth_callback = Phys_Envato_Hosted::get_url_verify_callback();//Back to this site
...
$subscription_code = Phys_Envato_Hosted::get_subscription_code();
?>

<form class="tc-form-verify-subscription-code"
      action="<?php echo esc_url( Phys_Envato_Hosted::get_url_verify_subscription_code() ); ?>"
      method="post">
    <input type="hidden" name="theme" value="<?php echo esc_attr( $theme ); ?>">
    <input type="hidden" name="core_version" value="<?php echo esc_attr( PHYS_CORE_VERSION ); ?>">
    ...
    <button class="button button-primary tc-button activate-btn tc-run-step"
            type="submit"><?php esc_html_e( 'Verify', 'phys-core' ); ?></button>
</form>
