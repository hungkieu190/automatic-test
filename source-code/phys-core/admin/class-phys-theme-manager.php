...
class Phys_Theme_Manager {
	...
	public static function set_metadata() {
		global $phys_dashboard;

		$wp_theme       = wp_get_theme();
		$stylesheet     = $wp_theme->get_stylesheet();
		$is_child_theme = phys_core_is_child_theme();
		 ...
		 * Set purchase link.
		 */
		$purchase_link          = apply_filters( 'phys_envato_link_purchase', false );
		$theme['purchase_link'] = $purchase_link;

		/**
		 * Set item id on themeforest.
		 */
		$item_id                 = apply_filters( 'phys_envato_item_id', false );
		...
		$changelog_file = apply_filters( 'phys_theme_changelog_file', $changelog_file );
		...
		$integrations_file = apply_filters( 'phys_theme_integrations_file', $integrations_file );
		...
		$links_default = array(
			'docs'      => '#',
			'knowledge' => 'https://physcode.com/knowledge-base/',
			'support'   => 'https://support.physcode.com/',
			'changelog' => false
		);

		$links          = apply_filters( 'phys_theme_links_guide_user', array() );
		 ...
		 * Child themes.
		 */
		$theme['child_themes'] = apply_filters( 'phys_core_list_child_themes', array() );

		$phys_dashboard['theme_data'] = $theme;
	}
	...
	public static function get_metadata( $refresh = false ) {
		...
		$theme_data = isset( $phys_dashboard['theme_data'] ) ? $phys_dashboard['theme_data'] : false;
	...
	public static function can_update() {
		...
		if ( ! $data ) {
			return apply_filters( 'phys_core_can_update_theme', false );
		}
		...
		return apply_filters( 'phys_core_can_update_theme', $can_update );
	}
}
