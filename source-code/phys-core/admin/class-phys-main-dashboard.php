...
/**
 * Class Phys_System_Status.
 *
 * @since 0.8.5
 */
class Phys_Main_Dashboard extends Phys_Admin_Sub_Page {
	/**
	 * @var string
	 *
	 * @since 0.8.5
	...
	public static function all_boxes() {
		if ( self::$boxes === null ) {
			...
			self::$boxes       = array(
				'appearance'    => array(
					'id'    => 'appearance',
					'title' => __( 'Quick settings', 'phys-core' )
				),
				'open_row'      => array(
					'title' => 'Open div'
				),
				...
				'documentation' => array(
					'id'    => 'documentation',
					'title' => __( 'Help & Support', 'phys-core' ),
				),
				'support-us'    => array(
					'id'    => 'support-us',
					'title' => __( 'Support us', 'phys-core' ),
				)
			);
			...
			if ( $integrations_file ) {
				self::$boxes['integrations'] = array(
					'id'    => 'integrations',
					'title' => $theme_data['name'] . ' ' . __( 'Integrations', 'phys-core' ),
				);
			}
			...
			if ( $envato_id ) {
				self::$boxes['updates'] = array(
					'id'    => 'updates',
					'title' => __( 'Welcome admin', 'phys-core' ),
				);
			}
			...
			if ( $changelog_file ) {
				self::$boxes['changelog'] = array(
					'id'    => 'changelog',
					'title' => __( 'What is the new?', 'phys-core' ),
					'links' => $theme_data['links']['changelog']
				);
			}
		}

		return apply_filters( 'phys_dashboard_all_boxes', self::$boxes );
	}

	/**
	 * Phys_System_Status constructor.
	 *
	 * @since 0.8.5
	 */
	...
	private function init_hooks() {
		...
		add_action( 'phys_dashboard_registration_box', array( $this, 'render_registration_box' ), 10 );
 		add_action( 'phys_dashboard_boxes_content', array( $this, 'phys_dashboard_boxes_content' ) );
	...
	public function render_registration_box() {
		...
		Phys_Dashboard::get_template( 'partials/registration.php' );
	}
	...
	public function handle_order_boxes_dashboard() {
		...
		update_option( 'phys_dashboard_order_boxes', $post_data );
	...
	private function render_box( $key ) {
		...
		if($args['template']){
			Phys_Dashboard::get_template( 'boxes/master.php', $args );
		}
	...
	public function phys_dashboard_boxes_content() {
		$boxes_default = array(
			'updates',
 			'open_row',
			'changelog',
