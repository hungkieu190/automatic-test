...
/**
 * Class Phys_Widget_Importer_Service
 *
 * @since 0.4.0
 */

if ( ! class_exists( 'Phys_Widget_Importer_Service' ) ) {
	class Phys_Widget_Importer_Service {
		/**
		 * Remove all widgets
		 *
		...
		public function import( $json_data, $widget_logic, $clear_current = true ) {
			...
			self::mapping_nav_menu();
 			do_action('phys_core_after_import');
		}
		...
		private static function mapping_nav_menu() {
			...
			$terms          = $phys_wp_import->processed_terms;
		...
		public static function map_so_settings_widget( $settings ) {
			...
			if ( is_array( $panels_info ) ) {
				...
				if ( $class === 'WP_Nav_Menu_Widget' ) {
					...
					$map_nav_id = Phys_Importer_Mapping::get_map_nav_menu( $nav_id );
