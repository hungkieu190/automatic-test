 ...
 * @since 0.8.5
 */
class Phys_System_Status extends Phys_Admin_Sub_Page {
	/**
	 * @var string
	 *
	 * @since 0.8.5
	...
	public function add_sub_page( $sub_pages ) {
		...
		$sub_pages['system-status'] = array(
			'title' => __( 'System Status', 'phys-core' ),
	...
	public static function get_environment_info() {
		if ( self::$environments === null ) {
			...
			// WP memory limit
			$wp_memory_limit = phys_core_let_to_num( WP_MEMORY_LIMIT );
			if ( function_exists( 'memory_get_usage' ) ) {
				$wp_memory_limit = max( $wp_memory_limit, phys_core_let_to_num( ini_get( 'memory_limit' ) ) );
			}
			...
			$info['post_max_size']      = phys_core_let_to_num( ini_get( 'post_max_size' ) );
			...
			// Test GET requests
			$response = phys_core_test_request( 'https://api.envato.com' );
			if ( $response['return'] ) {
				$response = phys_core_test_request( Phys_Admin_Config::get( 'host_downloads' ) . '/ping/' );

				if ( $response['return'] ) {
					$response = phys_core_test_request( Phys_Admin_Config::get( 'api_check_self_update' ) );
				}
			}
			...
			//Site key
//			$info['site_key'] = Phys_Product_Registration::get_site_key();
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'phys-developer-access' );
		wp_enqueue_script( 'phys-core-system-status', PHYS_CORE_ADMIN_URI . '/assets/js/system-status.js', array(
			'jquery',
			'phys-core-clipboard'
		), PHYS_CORE_VERSION );
	}
