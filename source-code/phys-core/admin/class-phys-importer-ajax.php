...
class Phys_Importer_AJAX {
	 ...
	 * @since 0.6.0
	 */
	private static $key_option_current_demo = 'phys_importer_current_demo';
	...
	public function handle_ajax_uninstall() {
		Phys_Importer_Service::reset_data_demo();
	...
	public function handle_ajax() {
		$res = new Phys_Import_REST_Response();

		try {
			if ( ! isset( $_POST['packages'] ) || count( $_POST['packages'] ) == 0 ) {
				throw Phys_Error::create( __( 'Invalid packages!', 'phys-core' ), 10 );
			}

			if ( ! isset( $_POST['step'] ) ) {
				throw Phys_Error::create( __( 'Invalid step!', 'phys-core' ), 10 );
			}
			...
			if ( 'start' == $step ) {
				if ( ! isset( $_POST['demo'] ) ) {
					throw Phys_Error::create( __( 'Invalid demo!', 'phys-core' ), 10 );
				}

				$demo  = sanitize_text_field( $_POST['demo'] );
				$demos = Phys_Importer::get_demo_data( true );

				if ( ! isset( $demos[ $demo ] ) ) {
					throw Phys_Error::create( __( 'Demo not found!', 'phys-core' ) );
				}
				...
				if ( $_initializeImporter ) {
					$res->status  = 'success';
					$res->step    = $packages[0];
					$res->message = esc_html__( 'Prepare Importer success', 'phys-core' );
				}
			} else {
				/**
				 * @var $res_step Phys_Import_REST_Response
				 */
				$res_step = $this->_step_by_step( $step );

				//do_action( 'phys_core_importer_next_step', $done, $next, $ext );
	...
	private function check_retry_import() {
		...
		$current_posts_pp       = get_option( 'phys_importer_posts_pp', 100 );
		$current_attachments_pp = get_option( 'phys_importer_attachments_pp', 10 );

		update_option( 'phys_importer_posts_pp', max( intval( $current_posts_pp / 2 ), 10 ) );
		update_option( 'phys_importer_attachments_pp', max( intval( $current_attachments_pp / 2 ), 1 ) );
	}
	...
	private function _initializeImporter( $demo, $packages = array() ) {
		do_action( 'phys_core_importer_start_import_demo', $demo );

		$packages = (array) $packages;
		$packages = apply_filters( 'phys_core_importer_prepare_packages', $packages );
		$this->_save_current_demo_data( $demo, $packages );
		update_option( 'phys_importer_prepare_wp_import', false );
	...
	private function _prepare_demo_content() {
		...
		Phys_Importer_Service::analyze_content( $xml );
	}
	...
	private function _get_dir_current_demo() {
		...
		$base_dir = apply_filters( 'phys_core_importer_base_path_demo_data', false, $current_demo );
		if ( ! $base_dir ) {
			return $current_demo['dir'];
		}

		return apply_filters(
			'phys_core_importer_directory_current_demo',
	...
	private function step_finish() {
		 ...
		 */
		$phys_wp_import = Phys_WP_Import_Service::instance( false );
		$phys_wp_import->set_menu_locations();
		...
		Phys_Importer::update_key_demo_installed( $demo_key );
		$this->import_end();
	...
	private function _update_demo_key_to_server( $demo_key ) {
		try {
			$api_url = Phys_Admin_Config::get( 'api_phys_market' ) . '/update-demo-version/';
			$body_args = array(
				'site_code' => Phys_Product_Registration::get_data_theme_register('purchase_token'),
				'version'  => $demo_key,
			);
	...
	function import_end() {
		...
		if ( class_exists( '\Phys_EL_Kit\Modules\Cache' ) ) {
			\Phys_EL_Kit\Modules\Cache::instance()->regenerate();
		}
	}
	...
	private function _update_settings() {
		...
		Phys_Importer_Service::settings( $settings_file );
	 ...
	 *
	 * @return mixed
	 * @return Phys_Import_REST_Response
	 * @throws Exception
	 * @since 1.10.1
	 *
	 * @see   step_plugins
	...
	private function _step_by_step( $step = '' ) {
		do_action( 'phys_core_importer_start_step', $step );

		$callback_function = 'step_' . $step;

		$callable = apply_filters( "phys_core_importer_step_$step", array( $this, $callback_function ) );

		if ( is_callable( $callable ) ) {
			return call_user_func( $callable );
		}

		throw Phys_Error::create( __( 'Not found function' . $callable, 'phys-core' ), 3 );
	}
	...
	public function step_plugins() {
		$response        = new Phys_Import_REST_Response();
		...
		try {
			...
			if ( ! ( $count_all_plugins ) || $index_plugin >= $count_all_plugins ) {
				...
				$response->message       = esc_html__( 'Finish step activate plugins', 'phys-core' );
			...
			$plugin = Phys_Plugins_Manager::get_plugin_by_slug( $plugin_slug );
 ...
 //							throw new Exception( $rs_active->get_error_message() );
							$response->message = __( "Can't install the plugin {$plugin->get_name()}", "phys-core" );
  						}
						...
						throw new Exception(
							sprintf(
								__( 'Installing %1$s plugin failed. %2$s', 'phys-core' ),
								$plugin->get_name(),
								$string
							)
						);
					...
					if ( $result_active ) {
						...
						throw new Exception(
							sprintf(
								__( 'Activate %1$s plugin failed. %2$s', 'phys-core' ),
								$plugin->get_name(),
								$string
							)
						);
	...
	public function step_download_demo_data() {
		...
		try {
			...
			do_action( 'phys_core_importer_next_step', 'plugins', '', '' );

			$response->step_finished = 1;
			$response->status        = 'success';
			$response->percentage    = 100;
			$response->message       = esc_html__( 'Download demo data successfully', 'phys-core' );
	...
	public function step_theme_options() {
		...
		try {
			...
			$result       = Phys_Importer_Service::theme_options( $setting_file );
			...
			if ( $response->step_finished == 1 ) {
				do_action( 'phys_core_before_importer_main_content' );
			}
		} catch ( Phys_Error $exception ) {
			$response->message = $exception->getMessage();
		}
	...
	public function step_main_content() {
		...
		try {
			$prepare_wp_import = get_option( 'phys_importer_prepare_wp_import', false );

			if ( ! $prepare_wp_import ) {
				$this->_prepare_demo_content();
				update_option( 'phys_importer_prepare_wp_import', true );
			...
			$result           = $phys_wp_import->import_posts( $fetch_attachment );
			 ...
			 */
			$phys_wp_import->backfill_parents();
			$phys_wp_import->backfill_attachment_urls();
			$phys_wp_import->remap_featured_images();
	...
	public function step_media() {
		$response       = new Phys_Import_REST_Response();
		$phys_wp_import = Phys_WP_Import_Service::instance();
		$result         = $phys_wp_import->import_posts( true );
		...
		$response->status        = 'success';
		$response->message       = esc_html__( 'Import media success', 'phys-core' );
		$response->percentage    = 100;
		$response->step_finished = 1;
	...
	public function step_widgets() {
		...
		try {
			...
			Phys_Importer_Service::widget( $widget_file, $widget_logic );
	...
	public function step_revslider() {
		...
		try {
			...
			Phys_Importer_Service::revslider( $revsliders );
	...
	private function _try_step(
		...
		do_action( 'phys_core_importer_try_step', $current, $ext );
	...
	private function _send_response(
		...
		echo '<!-- PHYS_IMPORT_START -->';
		header( 'Content-Type: application/json; charset=' . get_option( 'blog_charset' ) );
		echo wp_json_encode( $data );
		echo '<!-- PHYS_IMPORT_END -->';
