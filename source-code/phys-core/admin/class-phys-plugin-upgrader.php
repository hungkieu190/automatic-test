...
class Phys_Plugin_Upgrader {
	 ...
	 * @var string
	 */
	private static $tag = 'PHYS_CORE_CHECK_PLUGIN_UPDATE';
	 ...
	 * @var string
	 */
	private static $key_cached_information_plugins = 'phys_core_information_plugins';
	...
	/**
	 * Phys_Plugin_Upgrader constructor.
	 *
	 * @since 1.1.0
	 *
	 * @param $url_api
	...
	private function get_data_plugins() {
 		...
 		// add option phys_fix_check_update_plugin in another theme
	...
	private function get_data_plugins_remote() {
		if ( ! $this->url_api ) {
			return new WP_Error( self::$tag, __( 'url_api is invalid', 'phys-core' ) );
		}
		...
		$response = Phys_Remote_Helper::post(
			$this->url_api,
			array(
				'body' => array(
					'plugins' => $arr_slugs,
