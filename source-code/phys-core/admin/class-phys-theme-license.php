...
class Phys_Theme_License extends Phys_Admin_Sub_Page {
	...
	public function activate( $request ) {
		...
		try {
			...
			$api_url = Phys_Admin_Config::get( 'api_phys_market' ) . '/activate/';
			...
			// Is my theme.
//			if ( ! empty( Phys_Free_Theme::get_theme_id() ) ) {
//				$body_args['my_theme_id'] = Phys_Free_Theme::get_theme_id();
//			}
			...
			// get personal_tockem from database
 			$personal_token = Phys_Product_Registration::get_data_theme_register('personal_token') ? Phys_Product_Registration::get_data_theme_register('personal_token') : $personal_token;
			if ( ! empty( $personal_token ) ) {
				$purchase_codes = Phys_Envato_API::list_all_purchase_codes( $personal_token );
	...
	public function deactivate( $request ) {
		try {
			// Call API to server.
			$api_url = Phys_Admin_Config::get( 'api_phys_market' ) . '/deactivate/';
...
//			}
	...
	public static function save_data_by_theme_key( $key, $value ) {
		Phys_Product_Registration::set_data_by_theme( $key, $value );
	}
