...
/**
 * Class Phys_Child_Themes.
 *
 * @since 1.1.1
 */
class Phys_Child_Themes extends Phys_Admin_Sub_Page {
	 ...
	 * @since 1.2.0
	 *
	 * @return Phys_Child_Theme[]
	 */
	public static function child_themes() {
		if ( self::$child_themes === null ) {
			$themes = array();

			$input_child_themes = Phys_Theme_Manager::get_data( 'child_themes' );
			...
			foreach ( $input_child_themes as $args ) {
				$themes[] = new Phys_Child_Theme( $args );
			}
	...
	private function init_hooks() {
		...
		add_action( 'wp_ajax_phys_child_themes_action', array( $this, 'handle_ajax_action' ) );
	}
	...
	public function handle_ajax_action() {
		...
		$action = isset( $_REQUEST['phys_action'] ) ? $_REQUEST['phys_action'] : false;

		if ( empty( $slug ) || empty( $action ) ) {
			wp_send_json_error( __( 'Something went wrong!', 'phys-core' ) );
		}

		$result = new WP_Error( 'phys_core_them_not_found', __( 'This theme not exist!', 'phys-core' ) );
				...
				switch ( $action ) {
					case 'install':
						if ( $theme_status != 'not_installed' ) {
							wp_send_json_error( __( 'This theme has already installed!', 'phys-core' ) );
						}
					...
					case 'activate':
						if ( $theme_status == 'not_installed' ) {
							wp_send_json_error( __( 'This theme has not installed yet!', 'phys-core' ) );
						}
	...
	public function switch_theme_update_mods() {
		...
		if ( get_theme_mod( 'phys_core_extend_parent_theme', false ) ) {
			return;
		}
		...
		set_theme_mod( 'phys_core_extend_parent_theme', true );
	}
	...
	public function add_sub_page( $sub_pages ) {
		...
		$sub_pages[ $this->key_page ] = array(
			'title' => __( 'Child Themes', 'phys-core' ),
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'phys-child-themes', PHYS_CORE_ADMIN_URI . '/assets/js/child-themes.js', array( 'wp-util', 'jquery', 'backbone', 'underscore' ), PHYS_CORE_VERSION );
	...
	private function localize_script() {
		...
		wp_localize_script( 'phys-child-themes', 'tc_child_themes', $data );
	}
	...
	private function get_data_template() {
		...
		return array(
			...
			'ajax_action' => 'phys_child_themes_action'
		);
	}
}
