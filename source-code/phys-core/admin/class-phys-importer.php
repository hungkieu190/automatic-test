 ...
 * @since     0.1.0
 */
class Phys_Importer extends Phys_Admin_Sub_Page {
	/**
	 * @since 0.8.5
	 *
	 * @var string
	 ...
	 * @var string
	 */
	public static $key_option_demo_installed = 'phys_importer_demo_installed';
	...
	public static function get_import_packages() {
		$packages = array(
			'theme_options' => array(
				'title'       => esc_attr__( 'Customizer Options', 'phys-core' ),
				'description' => esc_attr__( 'Import theme options and rewrite all current settings.', 'phys-core' ),
			),
			'main_content'  => array(
				'title'       => esc_attr__( 'Main Content', 'phys-core' ),
				'description' => esc_attr__( 'Import posts, pages, comments, menus, custom fields, terms and custom posts.',
					'phys-core' ),
			),
			'media'         => array(
				'title'       => esc_attr__( 'Media File', 'phys-core' ),
				'description' => esc_attr__( 'Download media files.', 'phys-core' ),
				'required'    => 'main_content',
			),
			'widgets'       => array(
				'title'       => esc_attr__( 'Widgets', 'phys-core' ),
				'description' => esc_attr__( 'Import widgets data.', 'phys-core' ),
			),
			'revslider'     => array(
				'title'       => esc_attr__( 'Slider Revolution', 'phys-core' ),
				'description' => esc_attr__( 'Import Slider Revolution.', 'phys-core' ),
			),
		);

		return apply_filters( 'phys_core_importer_packages', $packages );
	}
	...
	public static function get_demo_data( $check_plugins_require = false ) {
		...
		if ( $prefix_folder_data_demo ) {
			$base_uri_demo_data  = trailingslashit( WP_CONTENT_URL ) . 'uploads/phys-data-demos/' . esc_attr( $prefix_folder_data_demo ) . '/';
			$base_path_demo_data = trailingslashit( WP_CONTENT_DIR ) . 'uploads/phys-data-demos/' . esc_attr( $prefix_folder_data_demo ) . '/';
		} else {
			$base_uri_demo_data  = apply_filters( 'phys_core_importer_base_uri_demo_data', $THEME_URI . '/inc/data/demos/' );
		...
		foreach ( $demo_data as $key => $demo ) {
			...
			$plugins_require_all = Phys_Plugins_Manager::get_slug_plugins_require_all();
			...
			foreach ( $plugins_require as $slug ) {
				$plugin = Phys_Plugins_Manager::get_plugin_by_slug( $slug );
	...
	private function init_hooks() {
		...
		add_action( 'wp_ajax_phys_importer', array( $this, 'handle_ajax' ) );
		add_action( 'wp_ajax_phys_importer_uninstall', array( $this, 'handle_ajax_uninstall' ) );
		add_filter( 'phys_dashboard_sub_pages', array( $this, 'add_sub_page' ) );
		add_action( 'admin_init', array( $this, 'reset_import_demo' ) );
		add_filter( 'phys_importer_memory_limit', array( $this, 'memory_limit_importer' ) );
		add_action( 'admin_action_phys_importer_demo_data', array( $this, 'show_current_demo_data' ) );
		add_action( 'wp_import_insert_post', array( $this, 'import_mail_chimp' ), 10, 4 );
		if ( apply_filters( 'phys_download_data_demo', true ) ) {
			add_action( 'phys_core_importer_next_step', array( $this, 'importer_download_demo_data' ), 10, 2 );
		}
	}
	...
	public function importer_download_demo_data( $done, $next ) {
		...
		$demo_data = Phys_Importer_AJAX::get_current_demo_data();
		$demo_key  = $demo_data['demo'];
		$url       = Phys_Admin_Config::get( 'demo_data' ) . esc_attr( $prefix_folder_data_demo ) . '/demos/' . $demo_key . '.zip';

		$package = Phys_File_Helper::download_file( $url );
		if ( is_wp_error( $package ) ) {
			throw Phys_Error::create( $package->get_error_message(), 8, __( 'Please try again later.', 'estatesy-demo-data' ) );
		}

		$path_file = trailingslashit( WP_CONTENT_DIR ) . 'uploads/phys-data-demos/' . esc_attr( $prefix_folder_data_demo ) . '/' . $demo_key . '.zip';

		$dir   = pathinfo( $path_file, PATHINFO_DIRNAME );
		$unzip = Phys_File_Helper::unzip_file( $package, $dir );
		if ( is_wp_error( $unzip ) ) {
			throw Phys_Error::create( $unzip->get_error_message(), 0, __( 'Please try again later.', 'phys-core' ) );
		}
	}
	...
	public function show_current_demo_data() {
		...
		$current = Phys_Importer_AJAX::get_current_demo_data();
		print_r( $current );
	}
	...
	public function reset_import_demo() {
		...
		delete_option( 'phys_importer_posts_pp' );
		delete_option( 'phys_importer_attachments_pp' );
	}
	...
	public function add_sub_page( $sub_pages ) {
		$sub_pages['importer'] = array(
			'title' => __( 'Import Demo', 'phys-core' ),
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script(
			'phys-importer',
			PHYS_CORE_ADMIN_URI . '/assets/js/importer/importer.min.js',
	...
	public function handle_ajax() {
		if ( function_exists( 'wp_raise_memory_limit' ) ) {
			wp_raise_memory_limit( 'phys_importer' );
		}

		$importer_ajax = new Phys_Importer_AJAX();
	...
	public function localize_script() {
		...
		$nonce = wp_create_nonce( 'phys-importer' );

		wp_localize_script( 'phys-importer', 'phys_importer_data', array(
			'nonce'                => $nonce,
			'url_ajax'             => admin_url( 'admin-ajax.php' ),
			'admin_ajax_action'    => admin_url( 'admin-ajax.php' ),
			'admin_ajax_uninstall' => admin_url( 'admin-ajax.php' ),
			'details_error'        => array(
				'title'     => __( 'The import demo content failed!', 'phys-core' ),
			...
			'uninstall_successful' => __( 'Uninstall demo content successful :]', 'phys-core' ),
			'uninstall_failed'     => __( 'Uninstall demo content failed. Please try again :]', 'phys-core' ),
			'confirm_close'        => __( 'Do you really want to close?', 'phys-core' ),
			'something_went_wrong' => __( 'Some thing went wrong. Please try again :]', 'phys-core' ),
		) );

		wp_localize_script( 'phys-importer', 'phys_importer', array(
			'demos'     => $demos,
