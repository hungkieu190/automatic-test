...
/**
 * Class Phys_Notification.
 *
 * @since 0.8.7
 */
if ( ! class_exists( 'Phys_Notification' ) ) {

	class Phys_Notification extends Phys_Singleton {
		/**
		 * @since 0.8.7
		 *
		 * @var string
		 */
		private static $key_options = 'phys_hide_notifications';
		...
		private function init_hooks() {
			add_action( 'phys_dashboard_notifications', array( __CLASS__, 'print_notifications' ) );
			add_action( 'admin_notices', array( __CLASS__, 'print_global_notifications' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
			add_action( 'wp_ajax_phys_hide_notification', array( $this, 'ajax_hide_notification' ) );
		}
		...
		public function enqueue_scripts() {
			wp_enqueue_script( 'phys-notifications', PHYS_CORE_ADMIN_URI . '/assets/js/notifications.js', array( 'wp-util', 'jquery', 'backbone', 'underscore' ), PHYS_CORE_VERSION );
		...
		private function localize_scripts() {
			wp_localize_script( 'phys-notifications', 'phys_notifications', array(
				'ajax' => admin_url( 'admin-ajax.php?action=phys_hide_notification' ),
			) );
		}
		...
		public static function print_global_notifications() {
			...
			$notifications = $phys_notifications;
			...
			foreach ( $notifications as $notification ) {
                ...
                <div class="tc-notice <?php echo esc_attr( $class ); ?>" data-id="<?php echo esc_attr( $notification['id'] ); ?>">
					...
					<?php if ( $notification['dismissible'] ): ?>
                        <button type="button" class="physcore-notice-dismiss button button-secondary" style="margin: 0 0 0.8em 10px;"><span>Dismiss this notice.</span></button>
					<?php endif; ?>
                </div>
				<?php
			}
		...
		public static function add_notification( $notification ) {
			...
			$phys_notifications[] = $notification;
		}
	}
}
