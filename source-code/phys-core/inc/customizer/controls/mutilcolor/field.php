...
class Multicolor extends Field {
	...
	public $type = 'phys-multicolor';

	public function init( $args ) {
		add_filter( 'phys_customizer_output_control_classnames', array( $this, 'output_control_classnames' ) );
		...
		new \PhysCode\Customizer\Field\Generic( $parent_control_args );
		...
		foreach ( $args['choices'] as $choice => $choice_label ) {
			...
			$classnames  = isset( $args['wrapper_attrs']['class'] ) ? $args['wrapper_attrs']['class'] . ' phys-group-item' : '{default_class} phys-group-item';
			$classnames .= 1 === $loop_index ? ' phys-group-start' : ( $loop_index === $total_colors ? ' phys-group-end' : $classnames );
			...
			$control_args = wp_parse_args(
				array(
					...
					'wrapper_attrs'  => array(
						'data-phys-parent-control-type' => 'phys-multicolor',
						'class'                         => $classnames,
					),
			...
			new \PhysCode\Customizer\Field\Color( $control_args );
		}
	}
	...
	public static function sanitize( $value ) {
		foreach ( $value as $key => $subvalue ) {
			$value[ $key ] = \PhysCode\Customizer\Field\Color::sanitize( $subvalue );
		}
	...
	public function output_control_classnames( $classnames ) {
		$classnames['phys-multicolor'] = '\PhysCode\Customizer\CSS\Multicolor';
