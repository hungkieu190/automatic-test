<?php
namespace PhysCode\Customizer\Modules;
...
class Base extends \WP_Customize_Control {
	...
	public function enqueue() {
		$script_info = include PHYS_CUSTOMIZER_DIR . '/build/customizer.asset.php';

		wp_enqueue_script( 'phys-customizer-control', PHYS_CUSTOMIZER_URI . '/build/customizer.js', array_merge( $script_info['dependencies'], array( 'jquery-ui-sortable', 'jquery', 'customize-base', 'customize-controls' ) ), $script_info['version'], true );
		wp_enqueue_style( 'phys-customizer-control', PHYS_CUSTOMIZER_URI . '/build/customizer.css', array( 'wp-components' ), $script_info['version'] );
	}

	protected function render() {
		$id    = 'customize-control-' . str_replace( array( '[', ']' ), array( '-', '' ), $this->id );
		$class = 'customize-control customize-control-phys-customizer customize-control-' . $this->type;
		...
		if ( ! isset( $this->wrapper_attrs['data-phys-customizer-setting'] ) ) {
			$this->wrapper_attrs['data-phys-customizer-setting'] = $this->id;
		}

		if ( ! isset( $this->wrapper_attrs['data-phys-customizer-setting-link'] ) ) {
			if ( isset( $this->settings['default'] ) ) {
				$this->wrapper_attrs['data-phys-customizer-setting-link'] = $this->settings['default']->id;
			}
		}
	...
	public function to_json() {
		...
		// The kirki-config.
		$this->json['physCustomizerConfig'] = $this->phys_config;

		// The option-type.
		$this->json['physCustomizerOptionType'] = $this->option_type;

		// The option-name.
		$this->json['physCustomizerOptionName'] = $this->option_name;
