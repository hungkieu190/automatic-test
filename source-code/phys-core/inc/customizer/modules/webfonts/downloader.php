...
//					return $font['name'];
		...
		foreach ( $fonts as $font ) {
			if ( in_array( $font['name'], $stored_names ) ) {
				$font['path'] = $stored_names[ $font['name'] ]['path'];
				$new_fonts[] = $font;
				continue;
			}
		...
		if ( $change ) {
			delete_option( 'phys_customizer_downloaded_ggfont_files' );
			update_option( 'phys_customizer_downloaded_ggfont_files', $new_fonts );
		}
