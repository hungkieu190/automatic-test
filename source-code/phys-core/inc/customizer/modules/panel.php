...
class Panel {
	...
	public function add_panel( $wp_customize ) {
		...
		$this->args['type'] = false === strpos( $this->args['type'], 'phys-' ) ? 'phys-' . $this->args['type'] : $this->args['type'];
		$this->args['type'] = 'phys-default' === $this->args['type'] ? 'default' : $this->args['type'];

		$wp_customize->add_panel(
			new \WP_Customize_Panel(
				$wp_customize,
				$this->id,
				apply_filters( 'phys_customize_panel_args', $this->args, $this->id )
			)
		);

		do_action( 'phys_customize_panel_added', $this->id, $this->args );
	}
