...
use PhysCode\Customizer\Utils\Helper;
use PhysCode\Customizer\Modules\CSS\Generator;
...
class Css {	
	...
	private static $css_handle = 'phys-customizer-styles';

	private static $inline_styles_id = 'phys-customizer-inline-styles';

	public function __construct() {
		add_action( 'phys_customizer_field_init', array( $this, 'field_init' ), 10, 2 );
		add_action( 'init', array( $this, 'init' ) );
	}

	public function init() {
		if ( ! apply_filters( 'phys_customizer_output_inline_styles', true ) ) {
	...
	public function print_styles_inline() {
		...
		if ( defined( 'PHYS_CUSTOMIZER_NO_OUTPUT' ) && true === PHYS_CUSTOMIZER_NO_OUTPUT ) {
			$should_print = false;
		}
		...
		$inline_styles_id = apply_filters( 'phys_customizer_inline_styles_id', self::$inline_styles_id );
	...
	public function enqueue_styles() {
		$args = array(
			'action' => apply_filters( 'phys_customizer_styles_action_handle', self::$css_handle ),
		);
	...
	public function print_styles() {
		...
		do_action( 'phys_customizer_dynamic_css' );
	}
