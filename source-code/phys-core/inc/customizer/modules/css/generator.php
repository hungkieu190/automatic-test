...
final class Generator {
	...
	private function __construct() {
		$googlefonts = new \PhysCode\Customizer\Utils\GoogleFonts();
	...
	public static function css( $field ) {
		...
		$field['phys_config'] = isset( $field['phys_config'] ) ? $field['phys_config'] : 'global';
		...
		self::$value = apply_filters( 'phys_get_value', get_theme_mod( $field['id'], $default ), $setting_name, $default, $option_type );

		$classname            = '\PhysCode\Customizer\Modules\CSS\Output';
		$field_output_classes = apply_filters( 'phys_customizer_output_control_classnames', [] );
		$field_output_classes = apply_filters( "phys_customizer_{$field['phys_config']}_output_control_classnames", $field_output_classes );

		if ( array_key_exists( self::$field_type, $field_output_classes ) ) {
			$classname = $field_output_classes[ self::$field_type ];
		}

		$obj = new $classname( $field['phys_config'], self::$output, self::$value, $field );

		return $obj->get_styles();
	}

	public static function styles_parse( $css = [] ) {
		$css = apply_filters( 'phys_customizer_styles_array', $css );
