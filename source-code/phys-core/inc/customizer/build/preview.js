...
/* harmony export */   "default": function() { return /* binding */ PhysPostMessageOutput; }
/* harmony export */ });
/* harmony import */ var _wordpress_hooks__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @wordpress/hooks */ "@wordpress/hooks");
/* harmony import */ var _wordpress_hooks__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_wordpress_hooks__WEBPACK_IMPORTED_MODULE_0__);

const physPostMessage = {
  fields: {},
  styleTag: {
    add: function (id) {
      id = id.replace(/[^\w\s]/gi, '-');

      if (null === document.getElementById('phys-postmessage-' + id) || 'undefined' === typeof document.getElementById('phys-postmessage-' + id)) {
        jQuery('head').append('<style id="phys-customizer-postmessage-' + id + '"></style>');
      }
    },
    addData: function (id, styles) {
      id = id.replace('[', '-').replace(']', '');
      physPostMessage.styleTag.add(id);
      jQuery('#phys-customizer-postmessage-' + id).text(styles);
    }
  },
  util: {
  ...
  css: {
    fromOutput: function (output, value, controlType) {
      ...
      styles = (0,_wordpress_hooks__WEBPACK_IMPORTED_MODULE_0__.applyFilters)('physCustomizerPostMessageStylesOutput', styles, value, output, controlType);

      if ('' === styles) {
        switch (controlType) {
          case 'phys-multicolor':
          case 'phys-sortable':
            ...
            _.each(value, function (val, key) {
              ...
              processedValue = physPostMessage.util.processValue(output, val);
              ...
              var customProperty = controlType === 'phys-sortable' ? output.property + '-' + key : output.property;
          ...
          default:
            if ('phys-image' === controlType) {
              value = !_.isUndefined(value.url) ? physPostMessage.util.backgroundImageValue(value.url) : physPostMessage.util.backgroundImageValue(value);
            }
            ...
            if (_.isObject(value)) {
              ...
              processedValue = physPostMessage.util.processValue(output, value);
  ...
  html: {
    fromOutput: function (output, value) {
      ...
      value = physPostMessage.util.processValue(output, value);
...
function PhysPostMessageOutput() {
  let styles = '';

  _.each(physPostMessageFields, function (field) {
    ...
    wp.customize(fieldSetting, function (value) {
      value.bind(function (newVal) {
        ...
        _.each(field.js_vars, function (output) {
          output.function = !output.function || 'undefined' === typeof physPostMessage[output.function] ? 'css' : output.function;
          field.type = field.choices && field.choices.parent_type ? field.choices.parent_type : field.type;

          if ('css' === output.function) {
            styles += physPostMessage.css.fromOutput(output, newVal, field.type);
          } else {
            physPostMessage[output.function].fromOutput(output, newVal, field.type);
          }
        });

        physPostMessage.styleTag.addData(fieldSetting, styles);
      });
    });
  });
}
